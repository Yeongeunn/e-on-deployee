...
stages {
        ...
        stage('Build Frontend') {
            steps {
                ...
                sh "docker build --no-cache --build-arg VITE_API_URL=${VITE_API_URL} -t ${DOCKERHUB_ID_TEXT}/e-on-frontend:v5 -f frontend/Dockerfile ./frontend"            }
        }
        ...
        stage('Push Images to Docker Hub') {
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'dockerhub-id', usernameVariable: 'USER', passwordVariable: 'PASS')]) {
                    ...
                    sh "docker push ${FE_IMAGE_NAME}:v5"
